...
class Seager:
    ...
    def play(self):
        ...
        """
        if self["t"] is not None and self["t"][0] == "0":
            self.captTime = 1
            return self.located("0")
    ...
    def lemma4(self, w, z, k):
        ...
        if d1 == 1:
            ...
            if d:   #If probed vk and d1 == 2 then return lemma 2 w's other children
                ...
                return case1(self, p, w, d1, k)

        elif d1 == 3:
            if d:   #If probed vk and d1 == 3 then case 4 (dk-1 and dk+1 non-empty)
                return case4(self, p, w, d1, k)

            else:
                return case2(self, p, w, d1, k)

        elif d1 == 4:
            if d:   #If probed vk and d1 == 4 then target in subset of Children(w, z)
                return self.lemma4(self.tDict[self["dk"][0]].parent, self.tDict[self["dk"][-1]].parent, k)
    ...
    def probe(self, v):
        ...
        if self["t"] is None:  self["t"] = [choice(list(self.tree.nodes))]  #Random starting location

        self["probeList"].append(v) #Add probed node to probe list
        self.captTime += 1          #Increase capture time by one
                    
        d  = shortest_path_length(self.tree, v, self["t"][self.captTime - 1])   #Get dist from node to target
    ...
    def siblings(self, w, z):
        ...
        """
        if self.tDict[w].parent != self.tDict[z].parent: return
